  ...
  // this callback gets executed when all required modules are loaded
  {
    function CustomNavBarMenuViewModel() {
        ...
        this.renderSMSPaymentTables = function () {
            require([CustomRegistry.SMSPaymentTables.viewmodel, CustomRegistry.SMSPaymentTables.view], function (viewModel, view) {
        ...
        this.rendermCommerceFiles = function(){
          require([CustomRegistry.Files.viewmodel, CustomRegistry.Files.view], function (viewModel, view) {
                var template = _.template(view);
                var node = document.getElementById("custom-page-main-content");
                $(node).html(template);
                ko.cleanNode(node);
        ...
        this.renderCodebook = function(){
            require([CustomRegistry.Codebook.viewmodel, CustomRegistry.Codebook.view], function (viewModel, view) {
                var template = _.template(view);
                var node = document.getElementById("custom-page-main-content");
                $(node).html(template);
                ko.cleanNode(node);
